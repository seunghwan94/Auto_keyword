...
icon_path   = 'utils/config/AP.ico'        # 아이콘 파일 경로
st_filename = 'app.py'               # app.py 파일 이름
console     =  True                  # 콘솔창 표시

# 포함 할 폴더들 (예: config, utils 등)
include_dirs = ['utils', 'page', 'crawler']   
...
datas += collect_data_files('konlpy') 
...
# 누락된 파일들을 명시적으로 추가 (경로를 일치시킵니다)
datas.append(('utils/config/google_sheets.json', 'utils/config'))
datas.append(('utils/config/stopwords.txt', 'utils/config'))
datas.append(('utils/config/AP.ico', 'utils/config'))  # 아이콘 파일 추가
